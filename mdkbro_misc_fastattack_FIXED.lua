...
AttackDropdown = Tabs.Misc:AddDropdown({
...
-- Đồng bộ tốc độ từ Choose Attack Speed vào Fast Attack thật
_G.FastAttackGravity_Mode = _G.FastAttackGravity_Mode or "Fast Attack"
local SpeedToCooldown = {
    ["Normal Attack"] = 0.25,
    ["Fast Attack"] = 0.15,
    ["Super Fast Attack"] = 0.05,
    ["NEJI Attack"] = 0.1
...
task.spawn(function()
    while task.wait(0.1) do
        local cfg = getgenv().FA_Config
        if typeof(cfg) == "table" then
            cfg.AttackCooldown = SpeedToCooldown[_G.FastAttackGravity_Mode] or 0.15
        end
    end
end)
        ...
        for _, Enemy in ipairs(Folder:GetChildren()) do
            if Enemy ~= Character and self:IsEntityAlive(Enemy) then
                local BasePart = Enemy:FindFirstChild("HumanoidRootPart")
                    ...
                    or Enemy:FindFirstChild(Config.HitboxLimbs[1])
...
function FastAttack:UseNormalClick(Character, Humanoid, Cooldown)
    ...
    -- Giữ cách đánh M1 hiện tại nhưng dùng Attack.Kill giống kiểu OK HUB
    -- để đưa nhân vật tới đúng vị trí mục tiêu trước khi gửi hit.
    local TargetModel = self.EnemyRootPart.Parent
    if TargetModel and Attack and Attack.Kill then
        pcall(function()
            Attack.Kill(TargetModel, true)
        end)
    end
...
-- Không hook getgc và không chạy thêm Fast 2 để tránh đánh đôi / spam RegisterHit.
